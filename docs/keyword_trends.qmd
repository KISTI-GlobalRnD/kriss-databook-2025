...
keyword curves on and off. All data are sourced from the comprehensive keyword time-series dataset
generated after term canonicalisation.

::: callout-tip
모든 곡선은 토큰 기반 비중(PPM; parts-per-million)을 나타냅니다. 포인트에 마우스를 올리면 각 용어의
점수(c‑TF‑IDF), 총 등장 빈도(토큰 수), 문서 커버리지(해당 용어가 1회 이상 등장한 문서 수)를 함께 확인할
수 있습니다.
:::
...
<div class="keyword-timeseries-defs">
  <p><strong>Token Share (PPM)</strong> · Parts per million of the cluster’s total unigram tokens for each term-year pair. \( \mathrm{PPM}_{c,y}(t) = 10^{6} \cdot \frac{n_{c,y}(t)}{N_{c,y}} \)</p>
  ...
  <p><em>Score</em>, <em>Frequency</em>, and <em>Document Coverage</em> appear inside the hover tooltips for each point.</p>
</div>
</div>
...
(() => {
  ...
  const metricLabel = "Token Share (PPM)";
    ...
    rowsByTerm.forEach((termRows, term) => {
      ...
      traces.push({
        ...
        hovertemplate:
          "<b>%{customdata[0]}</b><br>" +
          "Year: %{x}<br>" +
          "Token Share (PPM): %{y:.2f}<br>" +
          "Score: %{customdata[1]:.3f}<br>" +
          "Frequency (tokens): %{customdata[2]:,.0f}<br>" +
          "Document coverage: %{customdata[3]:,.0f}<extra></extra>",
        customdata: sorted.map(r => [term, r.score ?? null, r.frequency ?? null, r.doc_coverage ?? null]),
        mode: "lines+markers",
        line: { width: 2 },
        marker: { size: 6 }
      });
