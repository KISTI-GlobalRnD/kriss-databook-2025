...
description: "연구주제(클러스터)별 상위 키워드의 연도별 변화를 인터랙티브하게 확인할 수 있도록 구성하였다."
page-title: false
numbered: false
format:
  html:
...
본 사업에서는 연구주제(클러스터)별 상위 키워드의 연도별 변화(시계열)를 인터랙티브하게 확인할 수 있도록 탐색기를 구성하였다.
상단 선택 메뉴로 클러스터를 전환할 수 있으며, 범례(legend)에서 개별 키워드 곡선을 켜고 끌 수 있다.
모든 데이터는 키워드 정규화(canonicalisation) 이후 재집계한 키워드 시계열 산출물을 기반으로 한다.

::: callout-tip
모든 곡선은 연도·클러스터 단위에서 해당 키워드가 1회 이상 등장한 **문헌 수(문서 커버리지)**를 나타냅니다.
포인트에 마우스를 올리면 각 용어의 점수(c‑TF‑IDF), 총 등장 빈도(토큰 수), 전체 기간 문서 커버리지(한 문서당 최대 1회),
그리고 해당 연도의 클러스터 문헌 수(분모) 등을 함께 확인할 수 있습니다.
:::
...
<div class="keyword-timeseries-defs">
  <p><strong>Documents (year)</strong> · Number of documents in the selected cluster-year that mention the term at least once.</p>
  <p><strong>Cluster Documents (year)</strong> · Total number of documents in the selected cluster-year (context for the document counts).</p>
  <p><strong>Score</strong> · Cluster-specific c‑TF‑IDF weight used to rank keywords (constant across years).</p>
  <p><strong>Frequency</strong> · Total token occurrences of the keyword within the cluster (aggregated across the full period).</p>
  <p><strong>Document Coverage</strong> · Number of documents within the cluster that mention the keyword at least once (aggregated across the full period).</p>
  <p><em>Score</em>, <em>Frequency</em>, <em>Document Coverage</em>, and cluster-year document totals appear inside the hover tooltips for each point.</p>
</div>
</div>
...
<script>
(() => {
  const dataUrl = "assets/keyword_all_timeseries.csv.gz";
  ...
  const metricKey = "doc_coverage_year_val";
  const metricLabel = "Documents (year)";
  const maxSparklines = 40;
  const defaultVisibleTerms = 5;
  ...
  function getShortLabel(sample, clusterId) {
    return (
      cleanText(sample.label_ko_short_final) ||
      cleanText(sample.short_label_kr) ||
      cleanText(sample.cluster_micro) ||
      `Cluster ${clusterId}`
    );
  }

  function getLongLabel(sample) {
    return cleanText(sample.label_ko_long_final) || cleanText(sample.long_label_kr);
  }

  function summariseCluster(cluster) {
    const { id, rows } = cluster;
    const sample = rows[0] || {};
    const label = getShortLabel(sample, id);
    const longLabel = getLongLabel(sample);
  ...
  function buildTraces(cluster) {
    ...
    const traces = [];
    termSeries.forEach((series, index) => {
      const { term, rows: sorted } = series;
      traces.push({
        name: term,
        ...
        hovertemplate:
          "<b>%{customdata[0]}</b><br>" +
          "Year: %{x}<br>" +
          "Documents (year): %{y:,.0f}<br>" +
          "Cluster docs (year): %{customdata[4]:,.0f}<br>" +
          "Share of cluster-year docs: %{customdata[5]:.1%}<br>" +
          "Score: %{customdata[1]:.3f}<br>" +
          "Frequency (tokens): %{customdata[2]:,.0f}<br>" +
          "Document coverage (total): %{customdata[3]:,.0f}<extra></extra>",
        customdata: sorted.map(r => {
          const docCountYear = r[metricKey];
          const clusterDocsYear = r.cluster_year_docs ?? null;
          const share = Number.isFinite(docCountYear) && Number.isFinite(clusterDocsYear) && clusterDocsYear > 0
            ? docCountYear / clusterDocsYear
            : null;
          return [term, r.score ?? null, r.frequency ?? null, r.doc_coverage ?? null, clusterDocsYear, share];
        }),
        mode: "lines+markers",
        visible: index < defaultVisibleTerms ? true : "legendonly",
        line: { width: 2 },
        marker: { size: 6 }
      });
    });
  ...
  function updateChart(cluster) {
    ...
    const clusterLabel = getShortLabel(sample, cluster.id);
    ...
    const layout = {
      title: {
        text: `${cluster.id} · ${clusterLabel} · ${metricLabel}`,
        font: { size: 20 }
      },
      xaxis: {
        title: "Year",
  ...
  function renderSparklines(cluster, yearRange) {
    ...
    termsToRender.forEach(termData => {
      ...
      const svg = d3.create("svg")
        ...
        .attr("role", "img")
        .attr("aria-label", `${term} document count trend`);
      ...
      const firstValue = Number.isFinite(first[metricKey])
        ? Math.round(first[metricKey]).toLocaleString()
        : "-";
      const lastValue = Number.isFinite(last[metricKey])
        ? Math.round(last[metricKey]).toLocaleString()
        : "-";
  ...
  function initialiseControls(clusters) {
    const clusterOptions = Array.from(clusters.values())
      ...
      .map(cluster => {
        const sample = cluster.rows[0] || {};
        const label = getShortLabel(sample, cluster.id);
  ...
  async function loadData() {
    try {
      ...
      const rows = d3.csvParse(decompressed);
      ...
      rows.forEach(rawRow => {
        ...
        row.doc_coverage = row.doc_coverage ? Number.parseFloat(row.doc_coverage) : null;
        row.doc_coverage_year_val = row.doc_coverage_year_val ? Number.parseFloat(row.doc_coverage_year_val) : null;
        row.cluster_year_docs = row.cluster_year_docs ? Number.parseFloat(row.cluster_year_docs) : null;
        row.doc_ppm_series_val = row.doc_ppm_series_val ? Number.parseFloat(row.doc_ppm_series_val) : null;
      ...
      metaEl.innerHTML = "<p>데이터를 불러오지 못했습니다. ZIP 경로 또는 접근 권한을 확인하세요.</p>";
    }
  }
